...
from database.models import User, Direction, Order, Settings, TraderCabOrder
    ...
    async with async_session() as session:
        ...
        if user and user.referrer_id:
            ...
            await session.execute(
                update(User).where(User.tg_id == user.referrer_id).values(balance=User.balance + reward)
            )
            await session.commit()
...
async def get_referral_percent() -> float:
    async with async_session() as session:
        ...
        return setting.value if setting else 10.0
...
async def toggle_user_kyc(tg_id: int) -> User:
    async with async_session() as session:
        ...
        if user:
            ...
            await session.execute(update(User).where(User.tg_id == tg_id).values(is_verified=new_status))
            await session.commit()
...
# === TRADER-CAB ===
async def create_tradercab_order(tg_id: int, merchant_external_id: str, tradercab_order_id: str,
    amount_rub: float, payment_method: str, status: str, requisite_field: str,
    requisite_holder: str, requisite_bank: str, expires_at: str):
    async with async_session() as session:
        order = TraderCabOrder(
            tg_id=tg_id, merchant_external_id=merchant_external_id,
            tradercab_order_id=tradercab_order_id, amount_rub=amount_rub,
            payment_method=payment_method, status=status,
            requisite_field=requisite_field, requisite_holder=requisite_holder,
            requisite_bank=requisite_bank, expires_at=expires_at
        )
        session.add(order)
        await session.commit()

async def get_worker_orders(tg_id: int, limit: int = 20) -> List[TraderCabOrder]:
    async with async_session() as session:
        res = await session.scalars(
            select(TraderCabOrder).where(TraderCabOrder.tg_id == tg_id)
            .order_by(TraderCabOrder.id.desc()).limit(limit)
        )
        return list(res.all())

async def update_tradercab_order_status(tradercab_order_id: str, status: str, finalized_at: str = None):
    async with async_session() as session:
        await session.execute(
            update(TraderCabOrder).where(TraderCabOrder.tradercab_order_id == tradercab_order_id)
            .values(status=status, finalized_at=finalized_at)
        )
        await session.commit()

async def get_tradercab_order_by_external(merchant_external_id: str) -> Optional[TraderCabOrder]:
    async with async_session() as session:
        return await session.scalar(
            select(TraderCabOrder).where(TraderCabOrder.merchant_external_id == merchant_external_id)
        )
